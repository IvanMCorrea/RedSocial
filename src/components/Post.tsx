...
const Post = ({ post, userId, handleLike }: any) => {
  return (
    <article className="bg-slate-50 flex flex-col mx-auto border rounded-3xl mt-10 overflow-hidden w-3/5 p-5 max-w-[48rem]">
      <header className="flex justify-between w-full mb-3 flex-col md:flex-row">
        <div className="flex items-center">
          <img
            className="h-12 w-12 rounded-full mr-3"
            src={post.usernameId.avatar && post.usernameId.avatar}
      ...
      <section className="flex flex-col w-full mb-3">
        ...
        {post.image && (
          <img src={post.image} alt={`Post`} className="rounded-3xl w-full" />
        )}
      </section>
      <footer className="flex flex-col">
        <div className="group relative flex">
          <p className="w-full cursor-pointer">
            Likes: <span className="font-semibold">{post.likes.length}</span>
          </p>
          {post.likes && post.likes[0] && (
            <span className="absolute top-10 scale-0 transition-all rounded bg-gray-800 p-2 text-xs text-white group-hover:scale-100">
              {post.likes.map((like: UserModel) => (
                <div className="flex items-center" key={like._id}>
                  <img
